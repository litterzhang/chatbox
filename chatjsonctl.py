...
class Chat:
	# 初始化对话
	def __init__(self, _name, _sim_new=0.7, _sim_old=0.2):

		self.name = _name
		self.sim_new = _sim_new
		self.sim_old = _sim_old

		chatjsondb.init_db(self.name)
	...
	def __del__(self):
		...
		with open(os.path.join(COMS, '%s_%s' % (self.name, datetime.datetime.now().strftime("%Y%m%d %H%M%S"))), 'w', encoding='utf-8') as fw:
			coms_str = json.dumps(self.coms, ensure_ascii=False)
			fw.write(coms_str)
	...
	# 对话添加
	def coms_add_que(self, que_str, que, sim):
		self.coms.append({'que_str' : que_str, 'que' : que, 'que_sim' : sim})

	def coms_add_ans(self, ans, score):
		self.coms[-1]['ans'] = ans
		self.coms[-1]['score'] = score

	# 进行问题控制
	def chat_ctl_question(self, que_str, que, sim, anss, que_type=2):
		que_new = Chat.create_question(que_str, que_type)

		if que and sim > self.sim_old:
	...
	# 进行回答控制
	def chat_ctl_answer(self, que, sim, anss):
...
def create_anss(que_id):
	while True:
		...
		Chat.add_answer(que_id, ans_str, ans_type, ans_seed, ans_deg, ans_score)
...
def chat_ex():
	...
	# 加载问答库	
	chat = Chat(man_name, sim_new, sim_old)
	...
	while True:
		...
		# 进行问题匹配
		que, sim, anss = Chat.match_question(que_str)

		if not que or sim < sim_new:
			que_new_type = input_int(input_tip='\n这是一个新问题, 输入问题类型: ', default=2)
			que, sim, anss = chat.chat_ctl_question(que_str, que, sim, anss, que_new_type)

		chat.coms_add_que(que_str, que, sim)

		#这里进行答案匹配
		ans, score = chat.chat_ctl_answer(que, sim, anss)
		chat.coms_add_ans(ans, score)
		...
		if ans['id']<=0:
			...
			Chat.change_score(que['id'], ans['id'], ans_score)
